...
def new_window():
        ...
        label3=Label(tframe,text="Enter Test file path",bg="#edebeb",justify=LEFT)
        label2=Label(tframe,text="Enter Test case path",bg="#edebeb",justify=LEFT)
        entry1=Entry (tframe)
        entry2=Entry (tframe)
        entry3=Entry (tframe)
        def browsefunc():
            ...
            try:
                ...
                try:
                    ...
                    if(fc.cmp("file.txt",path,shallow=False)):  
                        ...
                        mc.execute("update scode set valid='y' where soid='{}'".format(id))
                        mydb.commit()
                    else:
        ...
        b=Button(win1,text="test",command=validate,bg="#edebeb",height=2,width=6,relief=RAISED)
        b1=Button(tframe,text="choose",command=browsefunc,bg="#edebeb",height=2,width=6,relief=RAISED)
        b2=Button(tframe,text="choose",command=browsefunc1,bg="#edebeb",height=2,width=6,relief=RAISED)
        tframe.pack(fill=X)
        label1.grid(row=0)
...
def new_window1():
        ...
        def reqcheck():
            id=entry1.get()
            mc.execute("select fmat from details d, scode s where d.moid=s.soid and s.mname='{}'".format(id))
            y=str(mc.fetchone())
            for i in bad_chars : 
                y =y.replace(i,'')
            mc.execute("select moid from details d, scode s where d.moid=s.soid and s.mname='{}'".format(id))
            x=str(mc.fetchone())
            for i in bad_chars : 
                x =x.replace(i,'')
        ...
        b=Button(win1,text="test",command=reqcheck,height=2,width=6,relief=RAISED)
        tframe.grid(row=0)
        label1.grid(row=0)
        entry1.grid(row=0,column=1)
        b.grid(row=1)
...
def new_window2():
    ...
    def insertval():
        ...
        vers=e5.get()
        st=e6.get()
        mc.execute("insert into scode values('{}','{}','{}','n')".format(id1,nam,st))
        mydb.commit()
        mc.execute("insert into details values('{}','{}','{}','{}')".format(path,id1,vers,fmt))
        mydb.commit()
        messagebox.showinfo("Entry","Successful")
    b=Button(win1,text="Insert",command=insertval,height=2,width=6,relief=RAISED)
    b1=Button(win1,text="Exit",command=win1.quit,height=2,width=6,relief=RAISED)
    b3=Button(win1,text="Refersh",command=lambda:[win1.quit,new_window2()],height=2,width=6,relief=RAISED)
    b2=Button(bframe,text="choose",command=browsefunc,bg="#edebeb",height=2,width=6,relief=RAISED)
    l1.grid(row=0)
...
b=Button(tframe,text="validation",command=new_window,height=2,width=6,relief=RAISED)
l=Label(tframe,text="This helps in check your program with test constrains",bg="#d4c253")
...
b1=Button(frame2,text="Proceed",command=new_window1,height=2,width=6,relief=RAISED)
...
b2=Button(frame3,text="Insert",command=new_window2,height=2,width=6,relief=RAISED)
l2=Label(frame3,text="Enter Values into Database to proceed further verification",bg="#c9b74f")
